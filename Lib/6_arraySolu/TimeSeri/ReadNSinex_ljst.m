...
while ~feof(fid)
    ...
    if strfind(tline,'[')
        ModelName = StandareStr_ljst(tline);
        ...
        while 1
            ...
            if length(StrInf) == 1     
                StrInf  = split(tline,' : ');
                VarName = StrInf{1};
                VarName = StandareStr_ljst(VarName);
                  ...
                  elseif StrInf{1}(1)  == '#'  %% 枝干
                      DataFlag = 2;
                      StructFieldStr = [ModelName StandareStr_ljst( StrInf{1}(3:end) )];
%                   else
%                       DataFlag = 3;  %% 叶子
%                       StructFieldStr = [StructFieldStr '.' StandareStr( StrInf{1}(2:end) )];
                ...
                if DataFlag ~= 2 && DataFlag ~= 1
                   StructFieldStr = [ModelName StandareStr_ljst( StrInf{1}(2:end) )];
                end
            end
            if DataFlag == 1
                VarName = StrInf{1};
    ...
    %             VarInf  = strrep(VarInf,'-',' -');
                ...
                %%
                [StrPer,NumPer] = StrAnalisy_ljst(VarInf);
                if NumPer == 1  & IsMinus    %
