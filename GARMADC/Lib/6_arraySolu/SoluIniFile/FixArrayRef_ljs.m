function FixFilePath = FixArrayRef(ObsPath,ConfigPath,SingleEpochData,SaveFixFilePath)
%% 获取数据存放位置
% 指定数据所在目录
Wildcard = '\*';
% 观测文件目录
...
FileStruct = FileExtract_ljs(ObsPath,IndexTag);
SvpTag  = '-svp.csv';
FileStruct = nTypeFileLink_ljs(FileStruct,ObsTag,ObsPath,SvpTag);
% 配置文件目录
ConfigTag  = '-initcfg.ini';
FileStruct = nTypeFileLink_ljs(FileStruct,ObsTag,ConfigPath,ConfigTag);
...
for DataNum = 1:FileStruct.FileNum
    ...
    ConfigData = ReadNSinex_ljs(INIFilePath);
    
%     [OBSData,SVP,INIData,MP1] = ReadData2Struct(OBSFilePath,SVPFilePath,INIFilePath);
%     MPList(DataNum).MP = MP1;

    Net = [];
    for len = 1:length(SingleEpochData{DataNum})
        Net = [Net,SingleEpochData{DataNum}(len,:)];
    end
...
[filepath,name,ext] = fileparts(SaveFixFilePath);
for DataNum = 1:FileStruct.FileNum
    INIReadFilePath     = FileStruct.SubFileList{3,DataNum};
    IniWriteFilePath    = ReplaceStr_ljs(INIReadFilePath,'initcfg',[name,'-fix']);
    SegIniWriteFilePath = split(IniWriteFilePath,'\');
    INIWriteFilePath    = [filepath,'\FixArrayCoor\',SegIniWriteFilePath{end-2},'\',SegIniWriteFilePath{end-1},'\',SegIniWriteFilePath{end}];
%     INIWriteFilePath = strrep(INIReadFilePath,'initcfg','fix');
    RWNSinex_ljs(INIReadFilePath,INIWriteFilePath,para,PostionNameList);
end

FixFilePath = [filepath,'\FixArrayCoor\',SegIniWriteFilePath{end-2},'\',SegIniWriteFilePath{end-1}];
