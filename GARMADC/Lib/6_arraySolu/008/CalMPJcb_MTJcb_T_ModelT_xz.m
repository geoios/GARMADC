function [dX,JCB] = CalMPJcb_MTJcb_T_ModelT_xz(OBSData,INIData,MP,MPNum,knots,spdeg,H)
...
for ShotNum = 1:N_shot
    ...
    e0 = ConstantV_Jcb(Transducer_ENU_ST,EachPos(index:index+2));
    e1 = ConstantV_Jcb(Transducer_ENU_RT,EachPos(index:index+2));
    kc0 = OBSData.kc0(ShotNum);
    Jcb(ShotNum,MPNum(1)-2:MPNum(1)) = -(e0 + e1)*kc0;
end
...
for GamNum = 1:length(GamTmp)
    ...
    for ShotNum = 1:N_shot
        ...
        [detalT1] = CalT_ZTD(OBSData.ST(ShotNum),[MP(1:MPNum(1)),GamTmp],MPNum,spdeg,knots,Z0,alpha0,INIData);
        [detalT2] = CalT_ZTD(OBSData.RT(ShotNum),[MP(1:MPNum(1)),GamTmp],MPNum,spdeg,knots,Z1,alpha1,INIData);
...
switch JcbMoodel
    ...
    case 3              %% 鑺傜偣涓偣闅忔満娓歌蛋绾︽潫 %%
        detalT_tmp=[];P= [];
        % 2.灏嗕腑闂存椂鍒诲甫鍏ュ埌B鏍锋潯鍩哄嚱鏁版眰铏氭嫙瑙傛祴
        Gam_Tmp = zeros(1,MPNum(end)-MPNum(1));
        for GamNum = 1:length(Gam_Tmp)
            Gam_Tmp(GamNum) = 1;
            % 鍒ゆ柇鍙傛暟鎵?灞炲尯闂?
            MPList = MPNum - MPNum(1);
            KnotsIndex = interp1([MPList realmax],0:length(MPList),GamNum','next','extrap'); %宸﹀紑鍙抽棴
            % 1.纭畾瑙傛祴鍖洪棿鑺傜偣涓棿鏃跺埢
        ...
        JCB = [JCB;VirObs];
        detalT     = [detalLOri; detalT_tmp];
        P = blkdiag(diag(Phi.^2),Mu*P);
        dx = inv(JCB'*P*JCB)*JCB'*P*detalT;
        dX(slvidx) = dx;
        
        
    case 4          %% 鍙傛暟杩炵画杩炵画鎬х害鏉熺瓥鐣? %%
