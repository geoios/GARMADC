...
for MuListNum =1:length(MuList)
    ...
    for BListNum = 1:length(BList)
        ...
        for DataNum = 1%:FileStruct.FileNum
            [OBSData,INIData,MP1] = ReadData2Struct_xz(OBSFilePath,INIFilePath);
            ...
            %3. 瑙ｇ畻
            [ModelT_MP_T,ModelT_MAXLoop2,ModelT_MP_T_OBSData] = TCalModelT_xz(soluType,OBSData,INIData,MP,MPNum,knots,spdeg,0);
        end
    end
end
