function [sin_sita,T,Y,Z,L ts xx zz LL,RayInf] = RayTracing_delay(PF,theta,T,Y,H)
...
for i = 1 : m-1 %i层声线跟踪
  ...
  if     H < inf       % Regarding the Depth as the cut-off condition
    TotalZ = sum(zz); 
    if TotalZ + 0.1 >= H ; V(3) = H - (TotalZ - zz(i)); end;                      % Calculate the propagation depth of the last layer
